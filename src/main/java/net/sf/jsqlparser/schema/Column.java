...
public class Column extends ASTNodeAccessImpl implements Expression, MultiPartName {
  ...
  /**
   * Retrieve the information regarding the {@code Table} this {@code Column} does belong to, if any can be inferred.
   * <p>
   ...
   * -     * </pre></blockquote>
   * -     * Here, we will get a {@code Table} object for a table called {@code t}. -     * But because the inference is local, such object will not know that {@code t} is -     * just an alias for
   * {@code Foo}.
   * <p>
   * -     * @return an instance of {@link net.sf.jsqlparser.schema.Table} representing the -     *          table this column does belong to, if it can be inferred. Can be {@code null}.
   */
